...
/**
 * Value-over-time area chart shared by the altitude and speed profiles (screen 1j): a filled
 * curve scaled between the min and max of [values] (one entry per point of [track], same size),
 * with those two bounds labelled below as [minLabel]/[maxLabel]. `track` must be chronologically
 * ordered and non-empty — callers should only show this card when there's data (older sessions
 * recorded before the track-points feature existed have none).
 *
 * Dragging horizontally moves a cursor along the curve, reporting the touched sample through
 * [onSelectedIndexChange] so callers can echo it elsewhere — the other profile and the route
 * trace mark the same point when they're fed the same [selectedIndex] / callback, which is what
 * keeps all three views in sync. The cursor stays put once the finger lifts, so the value can be
 * read hands-free.
 */
@Composable
fun TrackMetricProfile(
    track: List<TrackPoint>,
    values: List<Float>,
    lineColor: Color,
    selectedIndex: Int?,
    onSelectedIndexChange: (Int?) -> Unit,
    bubbleValueLabel: (Int) -> String,
    minLabel: String,
    maxLabel: String,
    ...
    Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Canvas(
            ...
            // y inverted since a higher value should draw higher on screen.
            val points = values.mapIndexed { index, value ->
                val fraction = (value - minValue) / range
            ...
            if (cursor != null) {
                ...
                val label = textMeasurer.measure(
                    text = "${bubbleValueLabel(index)}\n${formatTimeOfDay(track[index].timestamp)}",
                    style = TextStyle(
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Text(text = minLabel, fontSize = 11.sp, fontFamily = MonoFontFamily, color = ValueMuted)
            Text(text = maxLabel, fontSize = 11.sp, fontFamily = MonoFontFamily, color = ValueMuted)
        }
    }
}
